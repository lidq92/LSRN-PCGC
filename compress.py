...
def train(args):
    ...
    # val_dataset = PCSRDataset(args, status='val')
...
def encode_pc(args):
    ...
    for model_cls in range(8):
        ...
        if os.path.exists(trained_model_file):
            ...
            trained_model_size = os.path.getsize(trained_model_file)
            trained_model_sizes.append(trained_model_size)
    ...
    fs = '{}_bc{}_nl{}_D{}_p{}_lr{}_fsr{}_bs{}_e{}_{}_ppqs{}_pqs{}div{}.bs'
    bs_str = fs.format(args.dataset, args.base_channel, args.num_layers, args.D, 
                           ...
                           args.activation, args.ppqs, args.pqs[0], args.pqs[1])
    ppqs = args.ppqs
    ...
    with open(bitstream, 'wb') as fout:
        ...
        write_ints(fout, args.pqs)
        write_ints(fout, (D, base_channel, num_layers, len(trained_model_sizes), len(bin_sizes)))
        write_ints(fout, model_clses)
        write_ints(fout, trained_model_sizes)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-pqs', '--pqs', type=parse_pqs, default='4/3',
                        help=' (default: 4/3)')
    ...
    if args.pqs[0]/args.pqs[1] <=1:
        print('require pqs param must greater than 1 !!!!')
        sys.exit()
    ...
    fs = '{}_{}_bc{}_nl{}_D{}_p{}_lr{}_fsr{}_bs{}_e{}_{}_ppqs{}_pqs{}div{}'
    args.f_str = fs.format(args.model, args.activation, args.base_channel, args.num_layers, args.D, 
                           args.precision, args.lr, args.frame_sampling_rate, args.batch_size, args.epochs, 
                           args.dataset, args.ppqs, args.pqs[0], args.pqs[1])
