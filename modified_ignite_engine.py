...
def _prepare_train_batch(batch, device=None, non_blocking=False):
    ...
    return (convert_tensor(x, device=device, non_blocking=non_blocking), 
            ...
            convert_tensor(mask, device=device, non_blocking=non_blocking))
...
def create_supervised_trainer(model, optimizer, loss_fn, device=None, 
    ...
    def _update(engine, batch):
        optimizer.zero_grad()
        model.train()
        x, y, mask = prepare_batch(batch, device=device, non_blocking=non_blocking)
        y_pred = model(x)
...
def create_supervised_validator(model, metrics, device = None,
                                non_blocking = False, prepare_batch = _prepare_train_batch, 
                                output_transform = lambda x, y, y_pred, mask: (y_pred, y, mask)):
    metrics = metrics or {}
    def _inference(engine, batch):
        ...
        with torch.no_grad():
            ...
            return output_transform(x, y, y_pred, mask)
