...
datasetname_lst = list([
    'exercise_00000001_vox11.ply',
    'model_00000001_vox11.ply',
    'phil_139_vox10.ply',
    'sara_23_vox10.ply',
    'helicopter_00000001_vox10.ply',
    'clock1_vox11.ply',
    'candlestick_vox11.ply',
    'bicycle_vox11.ply',
    'QQdog_vox12.ply',
    'puppet_vox12.ply',
])
...
output = open(f'avs_Cat1_ppqs{args.ppqs}.txt', 'w')
file_lst = list()
for name in datasetname_lst:
    for pqs in pqs_lst:
        ...
        fs_base = 'decompress_{}_bc{}_nl{}_D{}_p{}_lr{}_fsr{}_bs{}_e{}_{}_ppqs{}_pqs{}div{}.log'
        if (pqs[0]/pqs[1]) >= 2:
            bc = 64 / (pqs[0] / pqs[1])
        else:
            bc = 32
        file = logpath+fs_base.format(name, int(bc), args.num_layers, args.D, args.precision, 
                                args.lr, args.frame_sampling_rate, args.batch_size, args.epochs, args.activation, args.ppqs, pqs[0], pqs[1])
...
excel_file = f'avs_Cat1_ppqs{args.ppqs}.xls' #
wbk.save(excel_file)
